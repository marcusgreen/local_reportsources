...
use local_reportsources\local\usagestats;

$id = required_param('id', PARAM_INT);

require_login();
$context = context_system::instance();
require_capability('local/reportsources:viewall', $context);

$q = query::get($id);
$rec = $q->record();

$PAGE->set_context($context);
$PAGE->set_url(new moodle_url('/local/reportsources/query_usage.php', ['id' => $id]));
$PAGE->set_pagelayout('admin');
$PAGE->set_title(get_string('usage:detailtitle', 'local_reportsources', format_string($rec->name)));
$PAGE->set_heading(format_string($rec->name));

$summary = usagestats::summary($id);
$viewdatefmt = get_string('strftimeviewdate', 'local_reportsources');

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('usage:detailtitle', 'local_reportsources', format_string($rec->name)));
...
if ($summary->views === 0) {
    echo $OUTPUT->notification(get_string('usage:nodata', 'local_reportsources'), 'info');
    echo $OUTPUT->footer();
    exit;
}
...
// Headline figures.
$cards = [
    [get_string('usage:views', 'local_reportsources'), $summary->views],
    [get_string('usage:uniqueviewers', 'local_reportsources'), $summary->viewers],
    [get_string('usage:firstviewed', 'local_reportsources'), userdate($summary->firstviewed, $viewdatefmt)],
    [get_string('usage:lastviewed', 'local_reportsources'), userdate($summary->lastviewed, $viewdatefmt)],
];
$cardhtml = '';
...
$chart->add_series(new \core\chart_series(get_string('usage:views', 'local_reportsources'), $trend->counts));
$chart->set_labels($trend->labels);
echo $OUTPUT->heading(get_string('usage:trend', 'local_reportsources'), 3);
echo $OUTPUT->render_chart($chart, false);

// Top viewers.
$topviewers = usagestats::top_viewers($id, 10);
if ($topviewers) {
    echo $OUTPUT->heading(get_string('usage:topviewers', 'local_reportsources'), 3);
    $table = new html_table();
    $table->head = [
        get_string('user'),
        get_string('usage:views', 'local_reportsources'),
        get_string('usage:lastviewed', 'local_reportsources'),
    ];
...
if (count($perreport) > 1) {
    ...
    echo $OUTPUT->heading(get_string('usage:perreport', 'local_reportsources'), 3);
    $table = new html_table();
    $table->head = [
        get_string('usage:report', 'local_reportsources'),
        get_string('usage:views', 'local_reportsources'),
        get_string('usage:lastviewed', 'local_reportsources'),
    ];
    $table->attributes['class'] = 'generaltable';
    ...
    foreach ($perreport as $report) {
        ...
        $label = ($name !== null && $name !== '')
            ? format_string($name)
            : get_string('usage:reportn', 'local_reportsources', $report->reportid);
    ...
    if ($deletedviews > 0) {
        $table->data[] = [
            get_string('usage:reportsdeleted', 'local_reportsources'),
            $deletedviews,
...
if ($recent) {
    echo $OUTPUT->heading(get_string('usage:recent', 'local_reportsources'), 3);
    $table = new html_table();
    $table->head = [get_string('user'), get_string('usage:when', 'local_reportsources')];
    $table->attributes['class'] = 'generaltable';
