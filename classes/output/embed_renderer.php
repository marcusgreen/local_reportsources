...
namespace local_reportsources\output;
...
class embed_renderer {
    ...
    public static function render_table(query $query, array $rows): string {
        if (!$rows) {
            return html_writer::tag('p', get_string('norows', 'local_reportsources'), ['class' => 'text-muted']);
        }
    ...
    public static function render_chart(query $query, array $rows, array $chartmeta, string $alt = ''): string {
        ...
        [$labels, $values] = query::chart_series($rows, $xcol, $ycol, $query->column_textcase($xcol));
