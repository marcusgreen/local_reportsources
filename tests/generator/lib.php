...
/**
 * Data generator for local_reportsources.
 *
 * Creates report-source query records for tests. Shared by PHPUnit (via
 * $this->getDataGenerator()->get_plugin_generator('local_reportsources')) and Behat (via the
 ...
 */
class local_reportsources_generator extends component_generator_base {
    ...
    public function create_query($record = null): stdClass {
        ...
        $id = $DB->insert_record('local_reportsources_query', (object) $record);

        if ($publish) {
            query::get($id)->publish();
        }

        return $DB->get_record('local_reportsources_query', ['id' => $id], '*', MUST_EXIST);
    }
}
