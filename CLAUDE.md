...
- Rejects unknown `%%…%%` tokens via `is_supported_token()`. Supported tokens (`%%WWWROOT%%`, `%%COURSEID%%`, `%%COURSECONTEXT%%`, `%%NOW%%`, `%%TIMESTAMP(expr[, format])%%`, `%%EPOCH(datetime)%%`) are exempt because they are substituted later in `view::resolve_placeholders()`, not at validate time
